...
단, 조건이 있는데 입력 날짜가 반드시 당일X, 내일 이후여야 한다.
...
여기서는 현재 날짜(java.time.LocalDate.now())에 1일을 더한 날짜로 설정되어 있어, 현재 날짜 이후의 날짜만 선택할 수 있게 하였다. onchange 속성은 사용자가 날짜를 선택한 후 발생하는 이벤트에 대한 핸들러를 정의한다.
